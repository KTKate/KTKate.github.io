...
try {
 ...
 for(const width of widths){
  ...
  for(const route of routes){
   ...
   const response=await page.goto(origin+route,{waitUntil:'networkidle'});await page.evaluate(()=>document.fonts.ready);await page.waitForTimeout(1700);
 ...
 const page=await browser.newPage({viewport:{width:390,height:900}});await page.goto(origin);
 ...
 const nojs=await browser.newPage({javaScriptEnabled:false,viewport:{width:320,height:900}});await nojs.goto(origin);assert.equal(await nojs.locator('.panel:visible').count(),4);assert.equal(await nojs.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await nojs.close();
 const linkResults=[];
 ...
 await page.goto(origin+'/work/nexus/');const nexus=await page.locator('main').innerText();assert(nexus.includes(note));assert(!/7 to 24|three.month|\bminutes\b|November|2026|IBM Z|8 hours/i.test(nexus));
 await page.goto(origin+'/print/');assert((await page.locator('main').innerText()).includes(note));assert.equal(await page.locator('.drafts').count(),1);assert.equal(await page.locator('.case-header h1').count(),3);
 for(const route of ['/work/nexus/','/work/linuxone-practice/','/work/hybrid-cloud/']){const html=await page.request.get(origin+route);const temp=await browser.newPage();await temp.setContent(await html.text());const paragraphs=await temp.locator('.case-body p').allTextContents();for(const p of paragraphs)assert((await page.locator('main').textContent()).includes(p),'Print body differs: '+route);await temp.close();}
 assert.equal((await page.request.get(origin+'/work/aiops-operating-model/')).status(),404);
 assert.equal((await page.request.get(origin+'/.private/linkedin-plan.md')).status(),404);
